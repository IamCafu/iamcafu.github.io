...
| Map pins and countries | `src/data/travel.ts`        |
| City map geometry      | `src/lib/kazakhstan-map.ts` |
| World map geometry     | `src/lib/world-map.ts`      |

Both maps are placed by HTML-comment markers in `src/data/about.ts`
(`<!-- travel-map -->` and `<!-- world-map -->`) — move a marker to move the
map, or delete it to remove the map.

- **City map**: adding a city needs only its name and coordinates in
  `src/data/travel.ts`. `visitedKazakhstanCities` draws in the accent color,
  `otherKazakhstanCities` draws smaller in the secondary accent for context. Set
  `labelPlacement` when a label would collide with a neighbour; a test fails if
  two pins land closer than 30 map units. To map a different country, replace
  the paths and projection constants in `src/lib/kazakhstan-map.ts`.
- **World map**: add an ISO 3166-1 alpha-3 code to `countryRelations` in
  `src/data/travel.ts` and that country lights up — `'home'` fills it with the
  secondary accent, `'visited'` with the primary one. `currentLocation` in the
  same file pins the "Now" marker from a latitude/longitude, and `countryFlags`
  fills a country with a flag from `public/images/flags/` instead of a flat
  color. All 239 country and territory paths ship in `src/lib/world-map.ts`
  (Natural Earth 1:50m, Miller cylindrical), so no regeneration is needed.
