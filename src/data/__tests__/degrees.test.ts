...
describe('degrees data', () => {
  ...
  it('detail links are valid URLs when present', () => {
    const urlRegex = /^https?:\/\/.+/;

    for (const degree of degrees) {
      for (const detail of degree.details ?? []) {
        if (typeof detail !== 'string') {
          expect(detail.link).toMatch(urlRegex);
        }
      }
    }
  });

  it('links the University of Ulsan thesis', () => {
    const ulsanDegree = degrees.find(
      (degree) => degree.school === 'University of Ulsan',
    );
    const thesis = ulsanDegree?.details?.find(
      (detail) =>
        typeof detail !== 'string' &&
        detail.text.includes('Automated Noise Removal'),
    );
