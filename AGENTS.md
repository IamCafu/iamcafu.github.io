...
- **About page extras**: `src/data/about.ts` places components with HTML-comment markers (`<!-- travel-map -->`, `<!-- world-map -->`); `Body` in `src/components/About/Sections.tsx` splits on the marker and splices the component in, so position is content-controlled and the comment never reaches the markdown renderer. Register new embeds in its `embeds` map
- **Maps/plotted data**: keep geographic source data in `src/data/` (lat/lon for points, ISO codes for countries) and generated geometry in `src/lib/` (`kazakhstan-map.ts`, `world-map.ts` — each documents its Natural Earth source, projection and simplification). Position point markers as HTML absolutely over the SVG so labels keep CSS pixel sizes and token-driven styling at any map width; use `vector-effect: non-scaling-stroke` for borders that must stay hairline as the map scales
- **CSS specificity inside prose**: `app/styles/pages/content.css` is imported after `styles/components/*`, so component styles nested in `.about-content` (lists, links) need a scoping class (e.g. `.travel-map .travel-map-pins`) to win
